...
public class ChronoPanel extends JPanel implements TimerListener {
	...
	static private String CHRONO_PANEL_SUFFIX_LOG = "[ChronoPanel]";
	...
	public void initButtons() {
		...
		chronoPanelLogger.info(String.format("%s PREAMBULE : %s", CHRONO_PANEL_SUFFIX_LOG, hasPreambuleMusic));
				...
				public void actionPerformed(ActionEvent e) {
					...
					preambuleMusicThread = new Thread(new Runnable() {
						...
						public void run() {
							...
							chronoPanelLogger.info(CHRONO_PANEL_SUFFIX_LOG + "Preambule have been started");
							System.out.println("PLAY!!! " + musicToPlay);
							player.play(salle.getPseudo() + "\\" + musicToPlay);
							
							String nextMusicToPlay = salle.getAmbianceMusique().get(musicNumber);
		...
		startButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (hasAmbianceMusic && !isPaused)
				{
					...
					String musicToPlay = salle.getAmbianceMusique().get(musicNumber);
					currentMusicLabel.setText("Musique en cours : " + musicToPlay);
					musicNumber++;
					if(musicNumber >= salle.getAmbianceMusique().size()) {
					...
					currentMusicThread = new Thread(new Runnable() {
						...
						public void run() {
							...
							chronoPanelLogger.info(CHRONO_PANEL_SUFFIX_LOG + "Start ambiance music " + musicToPlay);
							player.play(salle.getPseudo() + "\\" + musicToPlay);
						}
					});
				...
				if (hasBeginMusic && !isPaused && firstLaunch)
				{
					...
					new Thread(new Runnable() {
						...
						public void run() {
							System.out.println("PLAY!!! " + musicToPlay);
							chronoPanelLogger.info(CHRONO_PANEL_SUFFIX_LOG + "Start begin music " + musicToPlay);
							player.play(salle.getPseudo() + "\\" + musicToPlay);
						}
					}).start();
				}
...
//								+ " Nombre d'indice : " + session.getIndiceCount() + " \n" + session.getAllIndices());
		...
		refreshButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if ((musicNumber - 1) < salle.getAmbianceMusique().size()) {
					String musicToPlay = salle.getAmbianceMusique().get(musicNumber - 1);
					player.stop();
	...
	chronoPanelLogger.info(CHRONO_PANEL_SUFFIX_LOG + " Get session from website");
	  try {
		...
		if (all.trim().equals("nosession")) {
			...
			chronoPanelLogger.info(CHRONO_PANEL_SUFFIX_LOG + " Session " + session + " synchronisée, infos :");
			chronoPanelLogger.info(CHRONO_PANEL_SUFFIX_LOG + " " + firstname + " " + lastname);
			chronoPanelLogger.info(CHRONO_PANEL_SUFFIX_LOG + " Phone " + phone);
			chronoPanelLogger.info(CHRONO_PANEL_SUFFIX_LOG + " Players " + players);
			chronoPanelLogger.info(CHRONO_PANEL_SUFFIX_LOG + " Running " + running);
			chronoPanelLogger.info(CHRONO_PANEL_SUFFIX_LOG + " Price category " + price_category);
			chronoPanelLogger.info(CHRONO_PANEL_SUFFIX_LOG + " Special " + special);
			chronoPanelLogger.info(CHRONO_PANEL_SUFFIX_LOG + " Comment " + comment);
			chronoPanelLogger.info(CHRONO_PANEL_SUFFIX_LOG + " ANCV " + ancv);
			chronoPanelLogger.info(CHRONO_PANEL_SUFFIX_LOG + " Amount " + amount);
			chronoPanelLogger.info(CHRONO_PANEL_SUFFIX_LOG + " Payment type " + payment_type);
			...
			boolean unpaid = (!"0".equals(amount) && ("null".equals(payment_type) || "".equals(payment_type)));
			chronoPanelLogger.info("Unpaid ? " + unpaid);
	...
	public void showPaymentDialog(ChronoPanel chronoPanel) {
		PaymentDialog dialog = new PaymentDialog(getParent(), sessionMap, chronoPanel, chronoPanelLogger);
		dialog.openDialog();
	}
